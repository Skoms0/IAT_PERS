...
cfg.summary()
cfg.save()  # save config to ./1/config.json
...
class ConvAutoencoder(nn.Module):
    def __init__(self, cfg: MedNISTConfig):
        ...
        for in_c, out_c in zip(cfg.encoder_channels[:-1], cfg.encoder_channels[1:]):
            layers.append(nn.Conv2d(in_c, out_c, cfg.kernel_size, cfg.stride, cfg.padding))
            layers.append(getattr(nn, cfg.activation)(True))
        self.encoder = nn.Sequential(*layers)

        # --- Latent space ---
        self.fc1 = nn.Linear(cfg.encoder_channels[-1] * 4 * 4, cfg.latent_dim)
        self.fc2 = nn.Linear(cfg.latent_dim, cfg.encoder_channels[-1] * 4 * 4)

        # --- Decoder ---
        layers = []
        for in_c, out_c in zip(cfg.decoder_channels[:-1], cfg.decoder_channels[1:]):
            layers.append(nn.ConvTranspose2d(in_c, out_c, cfg.kernel_size, cfg.stride, cfg.padding))
            if out_c != 1:
                layers.append(getattr(nn, cfg.activation)(True))
            else:
                layers.append(getattr(nn, cfg.output_activation)())
...
for epoch in range(cfg.num_epochs):
    ...
    print(f"Epoch [{epoch+1}/{cfg.num_epochs}]  Train Loss: {train_loss:.6f}  Val Loss: {val_loss:.6f}")
